...
export default function TenantStore() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100">
      {/* Header da loja */}
      <div className="relative shadow-xl border-b tenant-bg-secondary tenant-border-primary">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          <div className="flex flex-col lg:flex-row items-start lg:items-center space-y-6 lg:space-y-0 lg:space-x-8">
            ...
            <div className="flex-1 space-y-4">
              <div>
                <h1 className="text-4xl lg:text-5xl font-bold tracking-tight tenant-text-primary">
                  {tenant.name}
                </h1>
                {tenant.description && (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                {tenant.contactEmail && (
                  <div className="flex items-center rounded-lg px-3 py-2 bg-gray-50">
                    <Mail className="h-5 w-5 mr-3 flex-shrink-0 tenant-text-primary" />
                    <span className="text-sm font-medium truncate tenant-text-primary">{tenant.contactEmail}</span>
                  </div>
                )}
                {tenant.contactPhone && (
                  <div className="flex items-center rounded-lg px-3 py-2 bg-gray-50">
                    <Phone className="h-5 w-5 mr-3 flex-shrink-0 tenant-text-primary" />
                    <span className="text-sm font-medium tenant-text-primary">{tenant.contactPhone}</span>
                  </div>
                )}
                {tenant.address && (
                  <div className="flex items-center rounded-lg px-3 py-2 bg-gray-50">
                    <MapPin className="h-5 w-5 mr-3 flex-shrink-0 tenant-text-primary" />
                    <span className="text-sm font-medium truncate tenant-text-primary">{tenant.address}</span>
                  </div>
                )}
                {tenant.domain && (
                  <div className="flex items-center rounded-lg px-3 py-2 bg-gray-50">
                    <Globe className="h-5 w-5 mr-3 flex-shrink-0 tenant-text-primary" />
                    <span className="text-sm font-medium truncate tenant-text-primary">{tenant.domain}</span>
                  </div>
                )}
              </div>
            </div>
